...
function mockFetch(url: string, init?: RequestInit): Response {
  ...
  if (url.endsWith('/api/auth/me')) return json({ id: 'mobius', display_name: 'x', role: 'admin' })
...
async function main() {
  ...
  fs.writeFileSync(path.join(TMP_HOME, 'login.json'), JSON.stringify({
    server: 'http://mock.local', username: 'mobius', token: 'tok', user: { id: 'mobius', display_name: 'x', role: 'admin' },
  }))
