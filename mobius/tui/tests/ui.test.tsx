...
async function testLogin() {
  ...
  installMock((url) => {
    ...
    if (url.endsWith('/api/auth/login')) return jsonResponse({ token: 'mock-jwt-token', user: { id: 'mobius', display_name: '付清旭', role: 'admin' } })
    return jsonResponse({ error: 'no mock' }, 404)
  })
  try {
    const { login } = await import('../src/api.js')
    const { saveLogin } = await import('../src/config.js')
    const r = await login('http://mock.local', 'mobius')
    ok(r.token === 'mock-jwt-token' && r.user.id === 'mobius', 'login() returns token + user')
    await saveLogin({ server: 'http://mock.local', username: 'mobius', token: r.token, user: r.user })
    const saved = JSON.parse(fs.readFileSync(path.join(TMP_HOME, 'login.json'), 'utf8'))
    ok(saved.token === 'mock-jwt-token' && saved.username === 'mobius', 'login.json persisted to temp home')
  ...
  try {
    ...
    stdin.write('mobius'); await delay(120)
    stdin.write('\t'); await delay(120)
    stdin.write('\r')
