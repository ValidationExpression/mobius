...
function mockFetch(url: string, init?: RequestInit): Response {
  ...
  if (url.endsWith('/api/auth/me')) return json({ id: 'mobius', display_name: '付清旭', role: 'admin', work_dir: '/tmp' })
  if (url.endsWith('/api/auth/login')) return json({ token: 'mock-jwt-token', user: { id: 'mobius', display_name: '付清旭', role: 'admin' } })
...
async function main() {
  ...
  fs.writeFileSync(path.join(TMP_HOME, 'login.json'), JSON.stringify({
    server: 'http://mock.local', username: 'mobius', token: 'mock-jwt-token',
    user: { id: 'mobius', display_name: '付清旭', role: 'admin' },
  }))
