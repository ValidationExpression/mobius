...
await ctx.addInitScript((t) => {
  ...
  try { localStorage.setItem('imac:first-login-tour-seen:v1:mobius', String(Date.now())); } catch(e){} // 关掉首登引导弹窗，否则盖住界面
}, TOKEN);
```
...
| 登录（密码免登） | `POST /api/auth/login {"username":"mobius"}` → `.token` |
| 登录态 localStorage | `cc-token` |
| 关首登引导 localStorage | `imac:first-login-tour-seen:v1:mobius` |
